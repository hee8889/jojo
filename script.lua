...
local AUTOEXEC_FILE    = "C:/Users/" .. USERNAME .. "/AppData/Local/Xeno/autoexec/BarameeHub.lua"

pcall(function()
    if not isfile(AUTOEXEC_FILE) then
        writefile(AUTOEXEC_FILE, AUTOEXEC_CONTENT)
        print("✓ Auto Execute บันทึกแล้ว: " .. AUTOEXEC_FILE)
    end
end)
...
local flags = {
    ESP=false, NOCLIP=false, FLY=false, FLYSpeed=40,
    RUN=false, RUNSpeed=20, StickFollow=false, StickDist=3,
    AutoAtk=false, AtkDelay=5, AutoSkill=false, SkillDelay=5,
    UseE=true, UseR=true, UseZ=true, UseX=true, UseC=true,
    AutoNearest=false, NearestDist=6, NearestAtkDelay=5,
    NearestDetectRange=100, AutoPlay=false, AutoPlayDelay=1,
}
...
local function createESP(player)
    ...
    bb.Name="ESP"; bb.Size=UDim2.fromOffset(200,60)
    ...
    lbl.TextScaled=true; lbl.Font=Enum.Font.SourceSansBold; lbl.Parent=bb
...
local Flying=false; local AlignOri,LinVel,FlyAttach
local MoveVec=Vector3.zero; local Ctrl={F=0,B=0,L=0,R=0,U=0,D=0}
...
local function startFly()
    ...
    AlignOri.Responsiveness=15; AlignOri.MaxTorque=30000; AlignOri.Parent=hrp
    LinVel=Instance.new("LinearVelocity")
    LinVel.Attachment0=FlyAttach
    LinVel.RelativeTo=Enum.ActuatorRelativeTo.World
    LinVel.MaxForce=25000; LinVel.Parent=hrp; Flying=true
end
...
LocalPlayer.CharacterAdded:Connect(function()
    stopFly(); task.wait(0.5); if flags.FLY then startFly() end
end)

local NoclipConn,Noclipping=nil,false; local ColCache={}
local function cacheCol(c) ColCache={}; for _,p in ipairs(c:GetDescendants()) do if p:IsA("BasePart") then ColCache[p]=p.CanCollide end end end
local function enableNC(c) for _,p in ipairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end
local function disableNC() for p,v in pairs(ColCache) do if p and p.Parent then p.CanCollide=v end end; ColCache={} end
...
local RunConn,Running=nil,false; local DefSpeed,CurHum=16,nil
local function applySpeed() if CurHum then CurHum.WalkSpeed=flags.RUNSpeed or DefSpeed end end
local function startRun()
    ...
    RunConn=RunService.Stepped:Connect(function()
        if not flags.RUN or h.Health<=0 then return end; applySpeed()
    end)
end
...
LocalPlayer.CharacterAdded:Connect(function(c)
    ...
    h.Died:Connect(stopRun); task.wait(0.3); if flags.RUN then startRun() end
end)

local function pressKey(keyCode)
    pcall(function()
        local vim=game:GetService("VirtualInputManager")
        vim:SendKeyEvent(true,keyCode,false,game); task.wait(0.05)
...
local function pressEnter()
    pcall(function()
        local vim=game:GetService("VirtualInputManager")
        vim:SendKeyEvent(true,Enum.KeyCode.Return,false,game); task.wait(0.05)
...
local SelectedTarget,StickConn=nil,nil
local AutoAtkActive,AutoSkillActive=false,false
local npcList,FilterDist={},nil
...
local function isTargetAlive()
    local hum=getTargetData(); return hum and hum.Health>0
end
local function getNearestNPC()
    ...
    for _,obj in ipairs(live:GetChildren()) do
        if obj:IsA("Model") and obj~=myChar and obj.Name~=LocalPlayer.Name then
...
AutoBox:AddToggle("AutoPlay", {
    ...
    Callback=function(v) flags.AutoPlay=v; Library:Notify(v and "Auto Play เปิดแล้ว" or "Auto Play ปิดแล้ว",2) end,
})
...
AutoBox:AddButton({
    ...
    Tooltip="บันทึก script ลง autoexec → Hub รันอัตโนมัติทุกครั้งที่ Rejoin",
    Func=function()
        pcall(function()
            writefile(AUTOEXEC_FILE, AUTOEXEC_CONTENT)
            Library:Notify("✅ เปิด Auto Execute แล้ว!\nHub จะรันอัตโนมัติทุกครั้งที่ Rejoin", 5)
        end)
    end,
})
AutoBox:AddButton({
    ...
    Func=function()
        pcall(function()
            if isfile(AUTOEXEC_FILE) then
                ...
                Library:Notify("⚠️ ไม่พบไฟล์ (ปิดอยู่แล้ว)", 3)
            end
        end)
    end,
})
AutoBox:AddButton({
    ...
    Func=function()
        pcall(function()
            if isfile(AUTOEXEC_FILE) then
                Library:Notify("✅ Auto Execute: เปิดอยู่", 4)
            else
                Library:Notify("❌ Auto Execute: ปิดอยู่", 4)
            end
        end)
    end,
})
    ...
    while task.wait(0.5) do
        ...
        if mm and mm.Enabled then
            ...
            if btn and btn.Visible then
                ...
                GuiService.SelectedObject=btn; task.wait(0.1); pressEnter()
                Library:Notify("✓ กด Quick Play แล้ว",2); continue
            end
        end
        ...
        if rc and rc.Enabled then
            ...
            if btn and btn.Visible then
                task.wait(flags.AutoPlayDelay or 1)
                GuiService.SelectedObject=btn; task.wait(0.1); pressEnter()
                Library:Notify("✓ กด Play Again แล้ว",2); continue
            end
        end
    end
end)
...
local function loadNPCList(maxDist)
    ...
    Options.TargetDrop:SetValues(opts); Options.TargetDrop:SetValue(opts[1])
...
local function startStick()
    ...
    StickConn=RunService.Heartbeat:Connect(function()
        ...
        myHRP.CFrame=CFrame.lookAt(tHRP.Position+Vector3.new(0,flags.StickDist,0),tHRP.Position)
    end)
end
local function stopStick() if StickConn then StickConn:Disconnect(); StickConn=nil end end
task.spawn(function()
    while task.wait() do
            ...
            if not AutoAtkActive then
                ...
                task.spawn(function()
                    while AutoAtkActive and flags.AutoAtk do
                        if not isTargetAlive() then Toggles.AutoAtk:SetValue(false); flags.AutoAtk=false; break end
                        clickM1(); task.wait((flags.AtkDelay or 5)*0.01)
                    end
                    AutoAtkActive=false
                end)
            end
            ...
            if not AutoSkillActive then
                ...
                task.spawn(function()
                    while AutoSkillActive and flags.AutoSkill do
                        if not isTargetAlive() then Toggles.AutoSkill:SetValue(false); flags.AutoSkill=false; break end
                        spamAllSkills(); task.wait((flags.SkillDelay or 5)*0.1)
                    end
...
RunService.Heartbeat:Connect(function()
    ...
    local myChar=LocalPlayer.Character; if not myChar then return end
    local myHRP=myChar:FindFirstChild("HumanoidRootPart"); if not myHRP then return end
    myHRP.CFrame=CFrame.lookAt(tHRP.Position+Vector3.new(0,flags.NearestDist,0),tHRP.Position)
end)

task.spawn(function()
    local tabOpened,lastTarget=false,nil
    while task.wait() do
        if not flags.AutoNearest then
            if tabOpened then pressKey(Enum.KeyCode.Tab); tabOpened=false; lastTarget=nil end
            continue
        end
        ...
        if alive then
            if not tabOpened or lastTarget~=tgt then
                pressKey(Enum.KeyCode.Tab); task.wait(0.1)
                tabOpened=true; lastTarget=tgt
            end
            clickM1(); spamAllSkills()
            task.wait((flags.NearestAtkDelay or 5)*0.01)
        else
            if tabOpened then pressKey(Enum.KeyCode.Tab); task.wait(0.1); tabOpened=false; lastTarget=nil end
            task.wait(0.2)
        end
    end
end)
...
ThemeManager:SetFolder("BarameeHub"); SaveManager:SetFolder("BarameeHub/game")
...
task.spawn(function() task.wait(3); loadNPCList(nil) end)
...
pcall(function()
    ...
    if isfile(AUTOEXEC_FILE) then
        ...
        Library:Notify("❌ Auto Execute: ปิดอยู่\nกด 'เปิด Auto Execute' ใน Auto Play tab",5)
    end
end)

print("✓ Baramee Hub Loaded! | User: "..USERNAME)
